...
bool BvhFileFormat::Read(SdfLayer* layer, std::string const& resolvedPath, bool /*metadataOnly*/) const
{
    ...
    for (size_t i = 0; i < document.m_JointNames.size(); ++i) {
        ...
        matrix.SetTranslate(GfVec3d(offset.m_Translation[0] * scale, offset.m_Translation[1] * scale, offset.m_Translation[2]) * scale);
    ...
    for (size_t frameIndex = 0; frameIndex < numFrames; ++frameIndex) {
        ...
        for (size_t jointIndex = 0; jointIndex < document.m_JointNames.size(); ++jointIndex) {
            ...
            localTransform.SetTransform(frameRotation, GfVec3f(static_cast<float>(frame.m_Translation[0] * scale), static_cast<float>(frame.m_Translation[1] * scale), static_cast<float>(frame.m_Translation[2] * scale)));
